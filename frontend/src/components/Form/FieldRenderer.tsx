...
export const FieldRenderer = <T extends Record<string, any>>({
  ...
  // ✅ Fetch options for search-select (with or without dependsOn)
  const fetchOptions = useCallback(async () => {
    if (!field.apiEndpoint) return;

    // If dependsOn exists, check parent value first
    if (field.dependsOn) {
    ...
    try {
      ...
      if (field.dependsOn) {
        endpoint = endpoint.replace(`{${field.dependsOn}}`, String(formValues[field.dependsOn as keyof T]));
      }
  ...
  useEffect(() => {
    if (field.type === 'search-select' && field.apiEndpoint) {
      fetchOptions();
    }
  ...
  const baseInputClass = `w-full px-4 py-2.5 border rounded-xl text-sm outline-none transition ${
    error
      ? 'border-red-300 focus:border-red-500 focus:ring-2 focus:ring-red-200'
      : 'border-slate-200 focus:border-medical-teal focus:ring-2 focus:ring-medical-teal/20'
  } ${
    isDisabled
      ? 'bg-slate-50 text-slate-400 cursor-not-allowed'
      : 'bg-white text-clinic-text'
  }`;
      ...
      <div className={colSpan}>
        ...
        <div className="relative">
          ...
          >
            ...
            <ChevronDown
              size={16}
              className={`text-slate-400 transition ${showDropdown ? 'rotate-180' : ''}`}
            />
          </button>
          {showDropdown && (
            <>
                  ...
                  allOptions.map((option) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 text-sm hover:bg-slate-50 transition ${
                        value === option.value
                          ? 'bg-medical-teal/5 text-medical-teal'
                          : 'text-clinic-text'
                      }`}
                    >
                      {option.label}
  ...
  // Default: text, email, password, number, tel, date, time, datetime-local
  const inputType = field.type === 'datetime-local' ? 'datetime-local' : field.type;

  // ✅ FIX: number 0 is NOT empty string
  const inputValue =
    field.type === 'number'
      ...
      <input
        ...
        onChange={(e) => {
          ...
          if (field.type === 'number') {
            // ✅ Send undefined if empty, Number if value exists
            setValue(field.name, raw === '' ? undefined : Number(raw));
          } else {
